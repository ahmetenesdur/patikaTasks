...
if(localStorage.getItem('todo'))    //LocalStorage varsa gir
{   
    ArrayTask = JSON.parse(localStorage.getItem('todo')) //LocalStorage çağır
...
function RemoveFunc(j) {                                        //"j" id numarası
    ...
    let index = ArrayTask.findIndex(function (Atask) {          //silinecek liste elemnını LocalStorage içindeki "index"ine ulaşmak
        ...
        localStorage.setItem('todo', JSON.stringify(ArrayTask)) //sildikten sonra tekrar set etmemiz lazım
        ArrayTask = JSON.parse( localStorage.getItem('todo') )
...
document.addEventListener('click', (element) =>                 //tıklanan liste elemanına ulaşma
{
    if(element.target.matches('li'))                            //liste elemanı mı diye kontrol
    {
        ...
        localStorage.setItem('todo', JSON.stringify(ArrayTask)) //değiştirdikten sonra tekrar set etmemiz lazım
        ArrayTask = JSON.parse( localStorage.getItem('todo') )
